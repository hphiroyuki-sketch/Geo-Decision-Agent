...
export function screeningToMarkdown(r: LeapReport): string {
  ...
  L.push(
    "> 各項目には根拠区分（衛星由来の算出値／現地確認済み／地図上で指定／登録・設定値／推定値／未取得）を付しています。**「未取得」「判定不可」の項目は判定していません。**",
  );
  L.push("");
  ...
  L.push("");
  L.push("TNFDが定める5つの特性について、本システムで判定できたものと、評価根拠が不足して判定していないものを区別して示します。");
  L.push("");
  L.push("| 基準 | 判定可否 | 結果 | 出典 | 備考 |");
  L.push("|---|---|---|---|---|");
  for (const s of r.sensitive) {
    const verdict = s.assessable ? "照合済み（参考）" : s.proxy ? "補助情報" : "**判定不可**";
  ...
  L.push("**判定可否の意味**");
  L.push("");
  L.push("- **照合済み（参考）**：公開データに照会し、結果を得た項目です。");
  L.push("- **補助情報**：当該基準を直接評価していない補助情報です。判定ではありません。");
  ...
  L.push("- 衛星データから特徴量の差を算出しています。生態系の変化の確定や、その**原因**（伐採・災害・病虫害・季節差）は判定できません。原因の特定には現地確認が必要です。");
  L.push(
  ...
  L.push(
    "- 生態系サービス供給上の重要性は、評価に必要なデータが未接続のため、直接評価していません。",
  );
